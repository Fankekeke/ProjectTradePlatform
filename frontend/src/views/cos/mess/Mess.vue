...
<!--        <a-button type="primary" ghost @click="add" v-hasPermission="['user:add']">新增</a-button>-->
      ...
      <a-table ref="TableInfo"
        ...
        <template slot="messContentShow" slot-scope="text, record">
          <a-tooltip>
            <template slot="title">
              {{ record.messContent }}
            </template>
            {{ record.messContent.slice(0, 75) }} ...
          </a-tooltip>
        </template>
...
<script>
  ...
  // import UserAdd from './MessAdd'
  // import UserEdit from './MessEdit'

  export default {
    name: 'User',
    components: {DeptInputTree, RangeDate},
    data() {
      return {
        advanced: false,
        userInfo: {
    ...
    computed: {
      columns() {
        ...
        return [{
          ...
          title: '留言内容',
          dataIndex: 'messContent',
          ellipsis: true,
          scopedSlots: { customRender: 'messContentShow' },
        }
          // {
          //   title: '操作',
          //   dataIndex: 'operation',
      ...
      batchDelete() {
        ...
        this.$confirm({
          ...
          onOk() {
            ...
            that.$delete('/cos/mess-info/'+ids,).then(() => {
