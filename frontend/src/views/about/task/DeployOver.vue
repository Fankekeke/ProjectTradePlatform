...
export default {
  ...
  methods: {
    ...
    onComplete () {
      ...
      let values = {orderCode: this.taskData.code}
      values.operatePic = projectImage.length > 0 ? projectImage.join(",") : null
      if (values.operatePic !== null) {
        this.$post('/cos/deploy-process/deploy', {
          ...values
        }).then((r) => {
          this.fileList = []
          this.$emit('finish')
